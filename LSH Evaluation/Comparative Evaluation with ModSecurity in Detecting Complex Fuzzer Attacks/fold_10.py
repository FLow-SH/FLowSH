...
def main():
    ...
    chunks = list(divide_chunks(get_list_of_lines(lines), int(len(get_list_of_lines(lines))/10)))
    if len(chunks) > 10:
        chunks = chunks[:-1]
    ...
    while i < len(chunks) - 1:
        ...
        train_chunk = []
        (precision, recall, f1_score, accuracy) = classify_LSH(lsh, dict_tokens, test_chunk, test_dict, float(threshold_value))
        metrics.append( (precision, recall, f1_score, accuracy))
        i += 1

    scores(
        f'./CSVs/W4AF/W4AF_metrics_8020_{threshold_value}.csv', metrics)
